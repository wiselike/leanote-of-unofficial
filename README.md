...
+ 12. 自研：将所有配置参数，调整为从系统全局变量中读取，而不是每次都从文件中读。优化了读取速度和效率
+ 13. 自研：使用gofmt格式化所有go代码，不对源码做任何手动改动
+ 14. 自研：禁用github.io，改为使用本地css文件
+ 15. 自研：禁用demo账号，自己用的话demo没有必要存在啊，直接用admin不就行啦
